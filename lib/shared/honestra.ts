...
export interface HonestraOptions {
  ...
  /**
   * Source type of the text being analyzed.
   * - "human": Text written by a human user (skip anthropomorphic_self patterns)
   * - "ai": Text generated by AI/model (check all patterns)
   * 
   * When analyzing human input, patterns like "I want", "I feel", "I prefer"
   * are legitimate expressions and should NOT be flagged as teleological.
   * These patterns are only problematic when AI pretends to have desires/feelings.
   */
 ...
 * @param sourceType - "human" or "ai" - determines which patterns to apply
 * 
 * When sourceType is "human", we skip anthropomorphic_self patterns
 * because humans legitimately have feelings, desires, and preferences.
 * These patterns are only problematic when AI pretends to have them.
 */
function detectTeleology(sentence: string, sourceType: SourceType = "ai"): string[] {
  const foundReasons: string[] = [];
  
  for (const pattern of teleologyPatterns) {
    // Skip anthropomorphic_self patterns for human input
    // Humans CAN legitimately say "I want", "I feel", "I prefer"
    if (sourceType === "human" && pattern.reason === "anthropomorphic_self") {
      continue;
    }
...
export function honestraFilter(
  ...
  const sourceType: SourceType = options.sourceType || "ai";
  ...
  for (const sentence of sentences) {
    const reasons = detectTeleology(sentence, sourceType);
